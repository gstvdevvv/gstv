...
export default async function LancamentosPage({
  ...
  function Secao({ titulo, cor, itens }: { titulo: string; cor: string; itens: typeof lancamentos }) {
    return (
      <div className="card p-4 overflow-x-auto">
        <div className="flex items-center justify-between mb-2">
          <p className="font-semibold" style={{ color: cor }}>{titulo}</p>
          <p className="font-semibold">{fmtBRL(totalRealizado(itens))}</p>
        </div>
        {itens.length === 0 ? (
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Secao titulo="💵 Receitas" cor="var(--receita)" itens={receitas} />
      <Secao titulo="📋 Despesas Fixas" cor="var(--foreground)" itens={fixas} />
      <Secao titulo="📈 Despesas Variáveis" cor="var(--accent)" itens={variaveis} />
    </div>
  );
}
